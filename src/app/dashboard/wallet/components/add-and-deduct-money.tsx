...
export const AddAndDeductMoney = ({
    wallet
}: {
    wallet: Wallet | null
}) => {
    ...
    return <div className="flex justify-between items-center gap-2">
        <ResponsiveModal open={addMoney} onOpenChange={setAddMoney}>
            <ResponsiveModalTrigger asChild>
                <Button size={"lg"} className="w-full sm:w-1/2 bg-gradient-to-r from-green-600 to-green-400 hover:from--700 hover:to-green-700 text-white font-semibold" disabled={isLoading}>
                    {isLoading ? <span><Loader2 className="animate-spin" /></span> : "Add Money"}
                </Button>
            </ResponsiveModalTrigger>
            <ResponsiveModalContent className="bg-gradient-to-br from-gray-900 to-black text-white">
                <ResponsiveModalHeader>
                    <ResponsiveModalTitle className="text-2xl font-bold">Add Money</ResponsiveModalTitle>
                ...
                <div className="relative">
                    ...
                    <Input min={0} type="number" placeholder="Enter amount" className="h-14 pl-10 bg-gray-800 border-gray-700 text-white placeholder-gray-400" value={amount == 0 ? "" : amount} onChange={(e) => setAmount(Number(e.target.value))} />
                </div>
                <Separator className="my-5 bg-gray-700" />
